...
public class Cliente_JuridicoController extends Application implements Initializable{
    ...
    void buscarCliente_Juridico(ActionEvent event) {
		...
		if (!idString.equals("")) {
			...
			if (cliente_juridico != null) {
				...
				label_label.setText(cliente_juridico.getId() + "");
				tx_nome_cliente_j.setText(cliente_juridico.getNome());
				tx_email_cliente_j.setText(cliente_juridico.getEmail());
				tx_telefone_cliente_j.setText(cliente_juridico.getTelefone() + "");
				tx_cnpj_cliente_j.setText(cliente_juridico.getCpf() + "");
				tx_cep_cliente_j.setText(cliente_juridico.getCep());
			}
    ...
    void excluiCliente_Juridico(ActionEvent event) {
    	...
    	if (result.get() == ButtonType.OK){
    		...
    		int qtde = new Cliente_JuridicoDao().deletar(clientej.getId());
    		listaCliente_Juridico();
    	}
    	limpaCampos();
    	}
    ...
    private Cliente_Juridico pegaDados() {
    	return new Cliente_Juridico( tx_nome_cliente_j.getText(), Integer.valueOf(tx_telefone_cliente_j.getText()), Integer.valueOf(tx_cnpj_cliente_j.getText()), tx_email_cliente_j.getText(), tx_cep_cliente_j.getText());
	}
	
    private Cliente_Juridico pegaDadosID() {
		return new Cliente_Juridico( Integer.valueOf(labelID.getText()),tx_nome_cliente_j.getText(), Integer.valueOf(tx_telefone_cliente_j.getText()), Integer.valueOf(tx_cnpj_cliente_j.getText()), tx_email_cliente_j.getText(), tx_cep_cliente_j.getText());
	}
	
    @FXML
    void BTNSairsistema(ActionEvent event) {
    	Stage stage = (Stage) btn_sair_cliente_j.getScene().getWindow();
    	stage.close();
    }
